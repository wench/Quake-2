...
{
	char	*value;
	bool ip = std::is_pod<qboolean>();
	bool tc = std::is_trivial<qboolean>();
	bool tct = std::is_trivially_copyable<qboolean>();
	bool sl = std::is_standard_layout <qboolean>();
	auto size = sizeof(qboolean);
	...
	// they can connect
	int clinum = (ent - g_edicts - 1);
	ent->client = game.clients + clinum;
...
void        ClientClick(edict_t* ent, vec3_t start, vec3_t end)
{
	...
	trace_t trace = gi.trace(start, vec3_origin, vec3_origin, end, world, MASK_ALL);
	if (trace.surface)
		Com_Printf("Surface \"%s\"\n", trace.surface->name);
	if (trace.ent) {
		Com_Printf("ent \"%s\"\n", trace.ent->classname);
		Com_Printf("{\n%s\n}\n", trace.ent->mapedict);
		trace.ent->s.effects |= (EF_COLOR_SHELL | EF_POWERSCREEN);
		trace.ent->s.renderfx |= (RF_SHELL_RED | RF_SHELL_GREEN | RF_SHELL_BLUE);
	}
