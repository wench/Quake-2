...
do {								\
	...
	desc->field = gi.TagMalloc(strlen(token)+1, TAG_GAME);	\
...
char *ANOX_parse_entity(char *classname, char *entities) 
{
	...
	desc = gi.TagMalloc(sizeof(anox_entity_desc_t), TAG_GAME);
	memset(desc, 0, sizeof(anox_entity_desc_t));

	// Copy classname
	desc->classname = gi.TagMalloc(strlen(classname)+1, TAG_GAME);
	...
	desc->model_path = gi.TagMalloc(i+1, TAG_GAME);
...
void ANOX_load_entity_descs() 
{
	...
	int length; 
	length = gi.FS_LoadFile("models/entity.dat", &data);
...
void anox_die_with_combat_anim (edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point)
{
	...
	self->anox = new_desc;
	self->classname = gi.TagMalloc(strlen(new_class)+1, TAG_LEVEL);
...
void anox_setup_moves(anox_entity_desc_t *desc)
{
	...
	desc->moves = gi.TagMalloc((info->num_framesets+2)*sizeof(mmove_t), TAG_GAME);
	all_frames = gi.TagMalloc(info->num_frames*sizeof(mframe_t), TAG_GAME);
	...
	if (!found_walk_a)
	{
		...
		desc->moves[i].frame = gi.TagMalloc(info->frameset[0].num_frames*sizeof(mframe_t), TAG_GAME);
