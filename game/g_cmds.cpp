...
#include "m_player.h"
	...
	*/
	...
	kill all hostiles to player

	argv(0)
	==================
	*/
	void Cmd_Kah_f(edict_t* ent)
	{
		char msg[256];
		...
		for (edict_t* from = g_edicts; from < &g_edicts[globals.num_edicts]; from++)
		{
			if (!from->inuse)
				continue;
			if (from->svflags & SVF_MONSTER && !(from->monsterinfo.aiflags & AI_GOOD_GUY))
			{
				T_Damage(from, ent, ent, vec3_origin, ent->s.origin, vec3_origin, 100000, 0, DAMAGE_NO_PROTECTION, MOD_UNKNOWN);
		...
		gi.cprintf(ent, PRINT_HIGH, "killed %i ents", i);
	}
