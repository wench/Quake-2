...
// go through all the dictionary pairs
	...
	size_t size = data - entstart;
	ent->mapedict = new (TAG_LEVEL) char[size];
	strncpy_s(ent->mapedict, size, entstart, _TRUNCATE);
	return data;
}
...
void SP_worldspawn (edict_t *ent)
{
	...
	char *apedata = 0, *data = 0, music[MAX_QPATH] = { 0 };
	int size =gi.FS_LoadFile(apename,(void**) &apedata);
	data = apedata;
	while (size>0)
	{
		...
		{
			strcpy_s(music, sizeof(music), "music/");
	...
	if (apedata)gi.FS_FreeFile(apedata);
		if (st.sky && st.sky[0])
