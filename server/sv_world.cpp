...
void SV_AreaEdicts_r(areanode_t* node)
{
	...
	edict_t* check;
	int			count;
	int types[] = { AREA_SOLID, AREA_TRIGGERS };

	count = 0;
	for(int t = 0; t < 2; t++)
	{
		int type = types[t];
		if (area_type != AREA_ALL && type != area_type)
			continue;

		// touch linked edicts
		if (type == AREA_SOLID)
			start = &node->solid_edicts;
		else if(type == AREA_TRIGGERS)
...
void SV_ClipMoveToEntities ( moveclip_t *clip )
{
	...
	num = SV_AreaEdicts (clip->boxmins, clip->boxmaxs, touchlist
		, MAX_EDICTS, clip->contentmask == -1?0:AREA_SOLID);
...
==================
*/
trace_t __vectorcall SV_Trace (vec3_t start, vec3_t mins, vec3_t maxs, vec3_t end, edict_t *passedict, int contentmask)
{
	moveclip_t	clip;
