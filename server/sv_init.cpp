...
static md2_info_t *SV_LoadAliasInfo (void *buffer, int modfilelen)
{
	...
	info = new(TAG_LEVEL) md2_info_t;
	info->num_framesets = 0;
	info->num_frames = num_frames;
	info->framename = new(TAG_LEVEL) char*[num_frames];
	...
	for (i = 0; i < num_frames; i++)
	{
		...
		info->framename[i] = new(TAG_LEVEL) char[len+1];
	...
	//
	info->frameset = new(TAG_LEVEL) md2_frameset_t[info->num_framesets];
...
static md2_info_t *SV_LoadMDAInfo (void *buffer, int modfilelen)
{
	...
	mdastring = new char [modfilelen+1];
	...
	while (1) 
	{
		...
		if (Q_stricmp(token, "basemodel") == 0)
		{
			...
			// Load the md2
			size = FS_LoadFile (token, (void**)&md2buf);
...
md2_info_t *SV_GetModelInfo (int index)
{
	...
	if (name[0] == '*')
	{
		...
		sv.md2_info[i] = info = new(TAG_LEVEL) md2_info_t;
		...
		info->frameset = new(TAG_LEVEL) md2_frameset_t;
	...
	size = FS_LoadFile(name, (void**)&buf);
...
void SV_InitGame (void)
{
	...
	svs.spawncount = rand();
	svs.clients = new client_t[maxclients->value];
