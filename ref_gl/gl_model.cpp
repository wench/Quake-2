...
void Mod_LoadVisibility (lump_t *l)
{
	...
	loadmodel->vis = (decltype(loadmodel->vis))Hunk_Alloc ( l->filelen);
...
void Mod_LoadVertexes (lump_t *l)
{
	...
	in = (decltype(in))(mod_base + l->fileofs);
	if (l->filelen % sizeof(*in))
		ri.Sys_Error (ERR_DROP, "MOD_LoadBmodel: funny lump size in %s",loadmodel->name);
	count = l->filelen / sizeof(*in);
	out = (decltype(out))Hunk_Alloc ( count*sizeof(*out));
...
void Mod_LoadEdges (lump_t *l)
{
	...
	if (l->filelen % sizeof(*in))
		ri.Sys_Error (ERR_DROP, "MOD_LoadBmodel: funny lump size in %s",loadmodel->name);
	count = l->filelen / sizeof(*in);
	out = (decltype(out))Hunk_Alloc ( (count + 1) * sizeof(*out));
...
//	glpoly_t	*poly;
	...
	in = (decltype(in))(void *)(mod_base + l->fileofs);
	if (l->filelen % sizeof(*in))
		ri.Sys_Error (ERR_DROP, "MOD_LoadBmodel: funny lump size in %s",loadmodel->name);
	count = l->filelen / sizeof(*in);
	out = (decltype(out))Hunk_Alloc ( count*sizeof(*out));
...
void Mod_LoadPlanes (lump_t *l)
{
	...
	if (l->filelen % sizeof(*in))
		ri.Sys_Error (ERR_DROP, "MOD_LoadBmodel: funny lump size in %s",loadmodel->name);
	count = l->filelen / sizeof(*in);
	out = (decltype(out))Hunk_Alloc ( count*2*sizeof(*out));	
...
void Mod_AutoGenAnoxProfile(model_t *mod, dmdl_anox_t *anox)
{
	...
	for (skin_counter = 0; skin_counter < skin_passes; skin_counter++)
	{
		...
		for (i = 0; i < anox->num_passes; i++)
		{
			...
			prof->skins[i].passes = pass = (decltype(pass))Hunk_Alloc(sizeof(mda_pass_t));
...
void Mod_LoadAliasModel (model_t *mod, void *buffer, qboolean gen_profiles)
{
	...
	if (version != ALIAS_VERSION && version != ALIAS_VERSION_ANOX_3_BYTE && 
		...
		version != ALIAS_VERSION_ANOX_OLD && version != ALIAS_VERSION_ANOX_CTC)
	...
	*/
	...
	pheader = (decltype(pheader))Hunk_Alloc (LittleLong(pinmodel->ofs_end) + total_extra);
...
mda_pass_t* Mod_ParsePass (model_t *mod, char **tokens, float minmax[2])
{
	int			i;
	char		*token;
	mda_pass_t	*pass =(mda_pass_t*) Hunk_Alloc(sizeof(mda_pass_t));
...
mda_skin_t* Mod_ParseSkin (model_t *mod, char **tokens, int *mask)
{
	...
	mda_pass_t	*pass = 0;
	mda_skin_t	*skin = (mda_skin_t*) Hunk_Alloc(sizeof(mda_skin_t));
...
mda_profile_t* Mod_ParseProfile (model_t *mod, dmdl_anox_t *anox, CHAR **tokens)
{
	...
	mda_skin_t	*skin = 0;
	mda_profile_t	*profile = (mda_profile_t*) Hunk_Alloc(sizeof(mda_profile_t));
...
void Mod_LoadMDAModel (model_t *mod, void *buffer)
{
	...
	mdastring = (char*)malloc (modfilelen+1);
	...
	while (1) 
	{
		...
		if (Q_stricmp(token, "basemodel") == 0)
		{
			...
			// Load the md2
			modfilelen = ri.FS_LoadFile (token,(void**) &md2buf);
...
void Mod_LoadSpriteModel (model_t *mod, void *buffer)
{
	...
	sprout = (decltype(sprout))Hunk_Alloc (modfilelen);
