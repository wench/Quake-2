...
void LoadPNG (char *name, byte **pic, int *width, int *height, qboolean hunk)
{
	...
	*/
	...
	row_pointers = (decltype(row_pointers))png_malloc(png_ptr, pheight*sizeof(png_bytep));

	*width = pwidth;
	*height = pheight;

	if (hunk) *pic = picture =(byte*) Hunk_Alloc2(pheight * pwidth*4);
	else *pic = picture = (byte*)malloc (pheight * pwidth*4);
...
void LoadPNG_8Bit (char *name, byte **pic, int *width, int *height, qboolean hunk)
{
	...
	*height = pheight;
	...
	if (hunk) *pic = picture =(byte*) Hunk_Alloc2(pheight * pwidth);
	else *pic = picture = (byte*)malloc (pheight * pwidth);
...
atd_frame_t * ATD_LoadAnimation_ParseFrame (char **tokens)
{
	...
	f = (decltype(f))Hunk_Alloc2(sizeof(atd_frame_t));
...
atd_t *ATD_LoadAnimation (char *tokens, byte **pic, int *width, int *height, int *clamp)
{
	...
	*clamp = false;
	...
	else *pic = (byte*)(atd->pixels = (int*)Hunk_Alloc2((*width)*(*height)*4));
