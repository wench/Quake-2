...
{
	// r_world_matrix
	int vx = floor(r_newrefdef.x * vid.width / vid.width);
	int x2 = ceil((r_newrefdef.x + r_newrefdef.width) * vid.width / vid.width);
	int vy = floor(vid.height - r_newrefdef.y * vid.height / vid.height);
	int y2 = ceil(vid.height - (r_newrefdef.y + r_newrefdef.height) * vid.height / vid.height);

	int w = x2 - vx;
	int h = vy - y2;
	int viewport[] = { vx, y2, w, h };
	double p[3] = { 0 };
	int result = gluUnProject(x, y, z, mvmatrix, projmatrix, viewport, p + 0, p + 1, p + 2);
	for (int i = 0; i < 3; i++) {
		point[i] = p[i];
	}
...
//		qglTranslatef ( gl_state.camera_separation, 0, 0 );
	...
	qglGetDoublev(GL_MODELVIEW_MATRIX, mvmatrix);
...
qboolean R_SetMode (void)
{
	...
	if ( vid_fullscreen->modified && !gl_config.allow_cds )
	{
		...
		ri.Cvar_SetValue( "vid_fullscreen", !vid_fullscreen->boolvalue);
		vid_fullscreen->modified = false;
	}

	fullscreen = vid_fullscreen->boolvalue;
...
{	
	...
	*/
		 ...
		 strstr( gl_config.extensions_string, "GL_SGI_compiled_vertex_array" ) )
	{
		...
		qglLockArraysEXT = (decltype(qglLockArraysEXT)) qwglGetProcAddress( "glLockArraysEXT" );
		qglUnlockArraysEXT = (decltype(qglUnlockArraysEXT)) qwglGetProcAddress( "glUnlockArraysEXT" );
	}
	else
	{
	...
	{
		qwglSwapIntervalEXT = (decltype(qwglSwapIntervalEXT)) qwglGetProcAddress( "wglSwapIntervalEXT" );
	...
	if ( strstr( gl_config.extensions_string, "GL_EXT_point_parameters" ) )
	{
		...
		{
			qglPointParameterfEXT = (decltype(qglPointParameterfEXT)) qwglGetProcAddress( "glPointParameterfEXT" );
			qglPointParameterfvEXT = (decltype(qglPointParameterfvEXT)) qwglGetProcAddress( "glPointParameterfvEXT" );
	...
	if ( strstr( gl_config.extensions_string, "GL_ARB_multitexture" ) )
	{
		if ( gl_ext_multitexture->value )
		{
			...
			qglMTexCoord2fSGIS = (decltype(qglMTexCoord2fSGIS)) qwglGetProcAddress( "glMultiTexCoord2fARB" );
			qglActiveTextureARB = (decltype(qglActiveTextureARB)) qwglGetProcAddress( "glActiveTextureARB" );
			qglClientActiveTextureARB = (decltype(qglClientActiveTextureARB)) qwglGetProcAddress( "glClientActiveTextureARB" );
	...
	if ( strstr( gl_config.extensions_string, "GL_SGIS_multitexture" ) )
	{
		...
		else if ( gl_ext_multitexture->value )
		{
			...
			qglMTexCoord2fSGIS = (decltype(qglMTexCoord2fSGIS)) qwglGetProcAddress( "glMTexCoord2fSGIS" );
			qglSelectTextureSGIS = (decltype(qglSelectTextureSGIS))qwglGetProcAddress( "glSelectTextureSGIS" );
	...
	if ( strstr( gl_config.extensions_string, "WGL_3DFX_gamma_control" ))
	{
		if ( gl_hardware_gamma->value )
		{
			...
			qwglGetDeviceGammaRamp = (decltype(qwglGetDeviceGammaRamp)) qwglGetProcAddress("wglGetDeviceGammaRamp3DFX");
			qwglSetDeviceGammaRamp = (decltype(qwglSetDeviceGammaRamp)) qwglGetProcAddress("wglSetDeviceGammaRamp3DFX");
	...
	else
	{
		...
		if (strstr(gl_config.extensions_string, "GL_ARB_fragment_program"))
		{
			if (gl_hardware_gamma->value)
			{
				...
				qwglSetDeviceGammaRamp =qwglSetDeviceGammaRampTexture;
			}
			else
			{
...
void R_BeginFrame( float camera_separation )
{
	...
	*/
	...
	if ( gl_log->modified )
	{
		GLimp_EnableLogging( gl_log->boolvalue);
