	...
	case WM_MOUSEMOVE:
		{
			...
			auto xPos = GET_X_LPARAM(lParam);    // horizontal position 
			auto yPos = GET_Y_LPARAM(lParam);    // vertical position 
...
qboolean VID_LoadRefresh( char *name )
{
	...
	if ( ( GetRefAPI = (decltype(GetRefAPI))GetProcAddress( reflib_library, "GetRefAPI" ) ) == 0 )
...
void VID_CheckChanges (void)
{
	...
	while (vid_ref->modified)
	{
		...
		** refresh has changed
		*/
		/* We don't support Software rendering in Aquakronox so don't even try */
