...
===========
*/
void IN_MouseEvent (int mstate, int xPos, int yPos)
{
	int		i;
...
void IN_MouseMove (usercmd_t *cmd)
{
	...
	// find mouse movement
	if (!GetCursorPos(&current_pos) ||!GetWindowRect(cl_hwnd, &window_rect))
		return;
	mx = current_pos.x - window_rect.left;
	my = current_pos.y -window_rect.top;
	mouse_x = mx;
	mouse_y = my;
	...
	{
		CL_changetogameimputmode(gim_move);
	}
	else
	{
		CL_changetogameimputmode(gim_interact);
		return;
	...
	//mx = current_pos.x - old_mouse_x;
	//my = current_pos.y - old_mouse_y;
	...
	old_mouse_x = current_pos.x;
	old_mouse_y = current_pos.y;
...
// add mouse X/Y movement to cmd
	...
	if (mx || my)
	{
		{
			SetCursorPos(window_center_x, window_center_y);
			old_mouse_x = window_center_x;
			old_mouse_y = window_center_y;
		}
	}
}
...
PDWORD RawValuePointer (int axis)
{
	...
	{
	default:
		Com_Error(ERR_FATAL,"invalid joystick axis %i",axis);
		return 0;
	case JOY_AXIS_X:
		return &ji.dwXpos;
	case JOY_AXIS_Y:
