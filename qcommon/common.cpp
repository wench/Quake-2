...
void MSG_ReadPosAccurate(sizebuf_t* msg_read, vec3_t pos)
{
...
void Qcommon_Shutdown (void)
{
  ...
  void* operator new (size_t bytes) throw() { return Z_Malloc(bytes); }
  void* operator new (size_t bytes, MemoryTag tag) throw() { return Z_TagMalloc(bytes, tag); }
  void operator delete (void* buf) throw() { return Z_Free(buf); }
  void operator delete (void* buf, MemoryTag tag) throw() { return Z_Free(buf); }