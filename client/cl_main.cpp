...
// wipe the entire cl structure
	for (i = 0; i < MAX_EDICTS; i++)
	{
		if (cl_entities[i].np[0].gen_rem) delete []cl_entities[i].np[0].gen_rem;
		if (cl_entities[i].np[1].gen_rem) delete []cl_entities[i].np[1].gen_rem;
		if (cl_entities[i].np[2].gen_rem) delete []cl_entities[i].np[2].gen_rem;
		if (cl_entities[i].np[3].gen_rem) delete []cl_entities[i].np[3].gen_rem;
	}
...
// 	Cmd_AddCommand ("packet", CL_Packet_f); // this is dangerous to leave in
	...
	Cmd_AddCommand("ClearParticles", CL_ClearParticles);
}
