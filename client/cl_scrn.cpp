...
void SCR_CenterPrint (char *str)
{
	...
	strncpy_s (scr_centerstring, sizeof(scr_centerstring), str, _TRUNCATE);
...
void SCR_ExecuteLayoutString (char *s)
{
	...
	while (s)
	{
		...
		{	// draw a ctf client block
			...
			sprintf_s(block,sizeof(block), "%3d %3d %-12.12s", score, ping, ci->name);
...
void SCR_DrawCursor()
{
	if (cls.gim == gim_interact || cls.key_dest != key_game)
	{
		re.DrawPic(mouse_x, mouse_y, "mouse_cursor");
...
//				return;
	...
	static int times[8] = { 0 };
	static int fn = 0;
	fn= (fn+1)&7;
	int cur = Sys_Milliseconds();
	int dif = (cur - times[fn & 7]);
	times[fn & 7] = cur;
	double fps = 8000.0 / dif;
	char msg[32];
	sprintf_s(msg, sizeof(msg), "FrameTime %ims %0.2f fps", dif/8, fps);
	for (int c = 0; c < strlen(msg); c++)
		re.DrawChar(c * 8 + 8, 8, msg[c]);
	SCR_DrawCursor();
	re.EndFrame();
