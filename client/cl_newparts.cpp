...
cnp_generator_t *CL_RegisterAPD (char *name)
{
	...
	while (1)
	{
		...
		else if (!Q_strcasecmp(token,"genrotate"))	
		{
			...
			gen->random_angle = strtol(token, NULL, 0)!= 0;
		}
		// gennorm : 4
		...
		else if (!Q_strcasecmp(token,"genflicker"))
		{
			...
			gen->flicker_all = strtol(token, NULL, 0)!=0;
		}
		// genfollow : 1
		else if (!Q_strcasecmp(token,"genfollow"))
		{
			...
			gen->follow = strtol(token, NULL, 0) !=0;
		}
		// genvol : 1 : 1 : 0 : 1
...
void CL_RunGenerators (centity_t *ent)
{
	...
	for (g = 0; g < 4; g++)
	{
		...
		if (ent->np[g].index != ent->current.np[g])
		{
			...
			if (ent->np[g].gen_rem) delete [] ent->np[g].gen_rem;
			...
			{
				ent->np[g].gen_rem = new float [gen->num_generators ];
